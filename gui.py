...
class PhaseSelectorWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Análise de Crescimento — GUI")
    ...
    def _on_span_move(self, xmin: float, xmax: float):
        ...
        dt = float(b - a)
        self.lbl_coords.setText(f"t₁ = {a:.4f} h  |  t₂ = {b:.4f} h  |  Δt = {dt:.4f} h")
    ...
    def install_selector(self):
        ...
        def on_select(xmin, xmax):
            ...
            self.lbl_coords.setText(f"t₁ = {a:.4f} h  |  t₂ = {b:.4f} h  |  Δt = {float(b-a):.4f} h")
    ...
    def on_open(self):
        ...
        from config.medium_aliases import resolve_medium_alias, canonical_medium_key  # noqa: F401 (import side-effects only)
    ...
    def on_export(self):
        ...
        for i, col in enumerate(self.columns):
            ...
            rows_reg.append({
                "Coluna": col, "Meio_ref": medium_for_col, "Fonte μ_ref": SOURCE,
                "R²": r2, "μ (h⁻¹)": mu, "t_d (h)": (np.log(2)/mu) if np.isfinite(mu) else np.nan,
                "t_d (min)": (np.log(2)/mu*60) if np.isfinite(mu) else np.nan,
                "Exp (h)": f"{self.th[j0]:.3f}–{self.th[j1]:.3f}",
